...
REMOTE_WORKER_GUIDANCE = (
    ...
    "Apply this policy to direct CLI requests, Telegram work, and Kanban tasks. "
    "Before independent builds, tests, lint, static analysis, browser QA, research, "
    "or repetitive processing likely to take 30 seconds or more, use the "
    "`remote-worker` skill and check `remote-worker status` and `remote-worker slots`.\n"
    "Run E2E UI tests with a visible headed browser on, in order: "
    "Seoyoon MacBook Air (`leeseoyoon` GUI session), then Mac mini. "
    "Playwright and Cypress runs must be headed. If neither remote GUI host is "
    "available, wait or block the work; never open the E2E browser on this MacBook Pro. "
    "Return and verify logs, reports, screenshots, and other test artifacts locally."
)

ORGANIZATION_KANBAN_SCOPE_GUIDANCE = (
    "# Organization Kanban reporting scope\n"
    "For questions about registered work, inspect only tasks assigned directly "
    "to the active Hermes profile. Use `hermes kanban --board crazyup-fillgaps "
    "list --mine`; do not use board-wide `stats`, an unfiltered `list`, or another "
    "profile's assignee filter, and do not mention foreign tasks discovered "
    "incidentally.\n"
    ...
    "ops_business, ops_infra; proposal_lead → proposal_designer, "
    "proposal_developer, proposal_qa; project_lead → project_designer, "
    "project_developer, project_qa. "
    "This internal Telegram fan-out is authorized only for the requested Kanban "
    "status report; it does not authorize customer or other external messages."
)

REPRESENTATIVE_ACTION_GUIDANCE = (
    "# Representative action cards\n"
    "The only human executor in this company is the representative. "
    "A task whose body contains the exact standalone line `사람 실행자: 대표` "
    "is waiting for that person to perform a real-world action: external "
    "send/submit/sign/pay/contract, physical device work (USB attach/detach, "
    "sideload APK, cable/SIM), or human-only OAuth consent "
    "(including Claude CLI login on the worker Mac, class "
    "`claude-cli-login`, reusable card "
    "`auth-login:claude-code:macbookpro`). "
    ...
    "Do not mark these as representative-action: dead profile reassignment, "
    "card set-model/unblock, credential isolation, skill visibility, "
    "non-secret runtime env such as GOOGLE_CLOUD_PROJECT, or PILOT_ACCEPTANCE "
    "canon. Those stay with the executive office or the stage owner.\n"
...
KANBAN_GUIDANCE = (
    ...
    "scope-creeping into the next thing. On the crazyup-fillgaps board the "
    "title must name the human-readable project (customer, product, or notice). "
    "Put a Wishket PID in parentheses only. Reject titles that are only "
    "`delivery 157366`, `PID 157680`, or `hunting 157624` plus a process word. "
    "Also put a one-line `프로젝트: <same name>` in the body.\n"
